    ...
    <main class="relative z-10 px-8 py-16">
      <div class="max-w-6xl mx-auto">
        <div class="text-center mb-12">
          <h1 class="text-4xl font-bold text-gray-900 mb-4">확정견적</h1>
          <p class="text-lg text-gray-600">고객이 수락한 견적들을 확인하세요</p>
        </div>
      ...
      <div class="bg-white rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div class="p-6">
          ...
          <div class="flex items-center justify-between mb-6">
            <h2 class="text-2xl font-bold text-gray-900">확정견적 상세</h2>
            <button 
          ...
          <div class="space-y-6">
            <!-- Customer Info -->
            <div>
              <h3 class="text-lg font-semibold text-gray-900 mb-3">고객 정보</h3>
              <div class="bg-gray-50 rounded-lg p-4">
                <p class="font-medium text-gray-900">{{ selectedQuote.customerName }}님</p>
              </div>
            </div>
...
const loadCompletedQuotes = async () => {
  ...
  try {
    ...
    // 파트너의 확정견적 가져오기
    const quotes = await firestoreService.getCompletedQuotesByPartner(user.currentUser?.uid || '')
    ...
    console.log('로드된 확정견적들:', completedQuotes.value)
  } catch (error) {
    console.error('확정견적 로드 실패:', error)
  } finally {
    loading.value = false
  }
}
